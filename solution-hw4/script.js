...
for (const option in glazeOptions) {
  ...
  glazeOption.text = glazeText[option];
  glazeOption.value = option;
...
function calcTotalPrice() {
  ...
  const glazePrice = glazeOptions[selectedGlaze];
  const packMultiplier = packOptions[selectedPack];
  ...
  const total = (basePrice + parseFloat(glazePrice)) * packMultiplier;
  return total;
}
...
function addToCart() {
  let rollObject = new Roll(rollType, glazeText[glazeDrop.value], packDrop.value, rolls[rollType].basePrice)
